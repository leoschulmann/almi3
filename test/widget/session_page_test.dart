...
import 'package:almi3/viewmodel/session_notifier.dart'
    show backlogWelcomeCopy, newLimitForkCopy;
...
Future<void> _insertDueCard(
  ...
  final lexemeProgressId = await db
      .into(db.lexemeProgressTable)
      .insert(
...
void main() {
  ...
  group('SessionPage', () {
    ...
    Widget harness({AppSettings? settings}) => ProviderScope(
      overrides: [
        ...
        settingsProvider.overrideWith(
          () => _SettingsNotifier(settings ?? AppSettings.defaultSettings()),
        ),
      ],
      child: MaterialApp(
    ...
    testWidgets('empty queue shows the "nothing to do" state, not a crash', (
      tester,
    ) async {
    ...
    testWidgets(
      'new-lexeme item shows the introduction screen with "Понятно"',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 10,
    ...
    testWidgets(
      'tapping "Понятно" introduces the lexeme and ends the (single-item) session',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 11,
    ...
    testWidgets(
      'mc4Recognition due card renders exactly 4 options and no rating control',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 20,
    ...
    testWidgets(
      'mc4Recognition: tapping the correct option grades as correct',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 24,
    ...
    testWidgets('mc4Recognition: tapping a distractor grades as incorrect', (
      tester,
    ) async {
      await _insertVerb(
        contentDb,
    ...
    testWidgets(
      'typedProduction due card renders a text field and no rating control',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 21,
    ...
    testWidgets(
      'answering a due card shows a health-delta reaction, then "Далее" advances',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 22,
    ...
    testWidgets(
      'typedProduction: a one-character-off answer is treated as correct-with-typo',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 26,
    ...
    testWidgets('typedProduction: a clearly wrong answer is graded incorrect', (
      tester,
    ) async {
      await _insertVerb(
        contentDb,
    ...
    testWidgets(
      'no FSRS internals (retention/stability/interval) ever appear',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 23,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          harness(
            settings: AppSettings.defaultSettings().copyWith(newCardsPerDay: 2),
          ),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(find.text(newLimitForkCopy(_l10n(tester))), findsOneWidget);
    ...
    testWidgets(
      'tapping "Продолжить с новыми" resumes the session with more new items',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 92,
    ...
    testWidgets('tapping "Потренировать" navigates to the practice page', (
      tester,
    ) async {
      await _insertVerb(
        contentDb,
    ...
    testWidgets('due-only session (no new items at all) never shows the fork', (
      tester,
    ) async {
      await _insertVerb(
        contentDb,
      ...
      expect(find.text(newLimitForkCopy(_l10n(tester))), findsNothing);
    });

    testWidgets(
      'cap not reached (fewer candidates than the norm) -> completes normally, no fork',
      (tester) async {
        ...
        await tester.pumpWidget(
          harness(
            settings: AppSettings.defaultSettings().copyWith(newCardsPerDay: 5),
          ),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      'backlog (>30 due) shows the welcome screen with no digits, then "Начать" starts the session',
      (tester) async {
        for (var i = 0; i < 35; i++) {
          await _insertVerb(
            contentDb,
        ...
        expect(find.text(backlogWelcomeCopy(_l10n(tester))), findsOneWidget);
        expect(find.text('Начать'), findsOneWidget);
        // No raw numbers anywhere in the welcome copy.
        expect(
          RegExp(r'\d').hasMatch(backlogWelcomeCopy(_l10n(tester))),
          isFalse,
        );
        ...
        expect(find.text(backlogWelcomeCopy(_l10n(tester))), findsNothing);
        expect(find.byType(TextField), findsOneWidget);
      },
    );

    testWidgets('no backlog (<=30 due) never shows the welcome screen', (
      tester,
    ) async {
      for (var i = 0; i < 10; i++) {
        await _insertVerb(
    ...
    testWidgets(
      '"Я знаю" grades an Easy review and shows the undo snackbar (matrix: "Я знаю")',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 120,
    ...
    testWidgets(
      'undo snackbar after "Я знаю" restores the card and deletes the log (matrix: "Undo после Я знаю")',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 121,
    ...
    testWidgets(
      '"Игнорировать" flags the lexeme and shows the undo snackbar (matrix: "Игнорировать")',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 122,
    ...
    testWidgets(
      'undo snackbar after "Игнорировать" flips status back to active (matrix: "Undo после Игнорировать")',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 123,
    ...
    testWidgets(
      '"Я знаю" on the LAST item defers the auto-pop while the undo window is open',
      (tester) async {
        await _insertVerb(
          contentDb,
          id: 124,
        ...
        await tester.pumpWidget(
          ProviderScope(
            overrides: [
              ...
              appDatabaseProvider.overrideWithValue(contentDb),
              settingsProvider.overrideWith(
                () => _SettingsNotifier(AppSettings.defaultSettings()),
              ),
            ],
            child: MaterialApp(
              home: const HomePage(),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.text('Начните заниматься, чтобы увидеть прогресс'),
          findsOneWidget,
        );
